...
const ProductFormModal: React.FC<ProductFormModalProps> = ({ product, categories, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-primary/80 backdrop-blur-xl border border-secondary/50 rounded-xl shadow-2xl w-full max-w-2xl p-8 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" name="name" value={formState.name} onChange={handleChange} className="w-full bg-background/50 border border-secondary rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-accent text-text-main" required />
          </div>
          <div>
            <label htmlFor="description" className="block text-sm font-medium text-text-muted mb-1">Deskripsi</label>
            <textarea name="description" value={formState.description} onChange={handleChange} rows={4} className="w-full bg-background/50 border border-secondary rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-accent text-text-main"></textarea>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="price" className="block text-sm font-medium text-text-muted mb-1">Harga (Rp)</label>
              <input type="number" name="price" value={formState.price} onChange={handleChange} className="w-full bg-background/50 border border-secondary rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-accent text-text-main" required />
            </div>
            <div>
              <label htmlFor="category" className="block text-sm font-medium text-text-muted mb-1">Kategori</label>
              <select name="category" value={formState.category} onChange={handleChange} className="w-full bg-background/50 border border-secondary rounded-md py-2 px-3 h-[42px] focus:outline-none focus:ring-2 focus:ring-accent text-text-main">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" name="size" value={formState.size} onChange={handleChange} className="w-full bg-background/50 border border-secondary rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-accent text-text-main" />
            </div>
             <div>
              <label htmlFor="inStock" className="block text-sm font-medium text-text-muted mb-1">Status</label>
               <div className="flex items-center h-[42px] bg-background/50 border border-secondary rounded-md px-3">
